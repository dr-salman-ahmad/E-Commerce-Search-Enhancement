...
class ProductSearch:
    ...
    def prepare_data(self, crop_limit):
        ...
        self.text_embeddings = [(self.feature_extractor.extract_text_features(self.text_to_embed(row)))[0].tolist() for
                                _, row in self.df.iterrows()]
        self.image_embeddings = [(self.feature_extractor.extract_image_features(row['image']))[0].tolist() for _, row in
                                 self.df.iterrows()]
    ...
    def add_data_to_vector_db(self):
        self.vector_db_handler.add_to_collection(image_embeddings=self.image_embeddings,
                                                 text_embeddings=self.text_embeddings, metadatas=self.metadatas,
                                                 ids=self.ids)
    ...
    def run(self, query: str, n_results: int = n_results):
        ...
        results = self.vector_db_handler.retrieve_products(query_embeddings=query_embeddings, n_results=n_results,
